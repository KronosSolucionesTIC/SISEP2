...
class financieroDAO extends UsuariosDAO
{
    ...
    public function getValorTotal()
    {
        ...
        $query = "select sum(valor_actividad) as total FROM `actividad` WHERE estadoV=1";
    ...
    public function getObjetivos()
    {
        ...
        $query = "select * FROM `objetivo` WHERE estadoV=1";
    ...
    public function getActividadTotal($pkID)
    {

        $query = "select IFNULL(sum(actividad_factura.valor),0) as total from actividad_factura
        left join actividad on actividad.pkID=actividad_factura.fkID_actividad
        left join factura on factura.pkID = actividad_factura.fkID_factura
        WHERE actividad_factura.estadoV =1 and factura.estadoV=1 and actividad.estadoV=1
        and actividad.pkID=".$pkID;

        return $this->EjecutarConsulta($query);
    }

    public function getValor($pkID_proyectoM)
    {

        $query = "select sum(valor_factura) as total FROM `factura` WHERE estadoV=1 and fkID_proyecto_marco=".$pkID_proyectoM;
